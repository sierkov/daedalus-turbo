...
for iter in `seq 1 3`; do
    for threads in 24 8; do
        ...
        docker run --cpus $threads --rm -v /data/cardano:/data/cardano dt /bin/bash -c "sudo chown -R dev:dev /data; DT_LOG=/data/cardano/dt-sync-http-$iter-$threads.log DT_WORKERS=$threads ./dt sync-http /data/cardano"
