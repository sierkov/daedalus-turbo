...
static std::array<cardano_hash_28, 50> top_stake_keys = {
...
static void warn_top_stake_key(const buffer &addr)
{
    ...
    if (it != top_stake_keys.end()) {
        cerr << "WARNING: The key " << addr << " is in the top " << top_stake_keys.size() << " by the number of transactions!\n"
...
int main(int argc, char **argv) {
    ...
    switch (addr_buf.size()) {
        case 28:
            warn_top_stake_key(addr_buf);
            cout << r.reconstruct_raw_addr(addr_buf);
            break;

        case 29:
            warn_top_stake_key(buffer(addr_buf.data() + 1, addr_buf.size() - 1));
