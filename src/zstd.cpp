...
static compress_result compress_chunk(const string &path, int level)
{
    compress_result res(path);
    try {
        uint8_vector orig, compressed;
        read_whole_file(path, orig);
        zstd_compress(compressed, orig, level);
        write_whole_file(path + ".zstd", compressed);
...
static compress_result decompress_chunk(const string &path, const string &dst_path)
{
    ...
    try {
        ...
        zstd_decompress(orig, compressed);
...
{
    if (argc < 3) {
        cerr << "Usage: zstd compress|decompress <chunks-dir> [<compression-level>]" << endl;
        return 1;
    }
    ...
    if (cmd == "compress") {
        ...
        for (const auto &path: chunks) sched.submit("compress", 0, compress_chunk, path, level);
    } else if (cmd == "decompress") {
        vector<pair<string, string>> chunks;
        for (const auto &entry : filesystem::directory_iterator(db_path)) {
            if (entry.path().extension() != ".zstd") continue;
