    ...
    "ed25519"_test = [] {
        "verify"_test = [] {
            ...
            std::array<uint8_t, 84> msg { 0xa3, 0x00, 0x81, 0x82, 0x58, 0x20, 0xc1, 0xa1, 0xff, 0x8e, 0x54, 0x99, 0xc3, 0x9f, 0xfa, 0x4c,
                                     ...
                                     0x00, 0x0a, 0xae, 0x60 };
            auto hash = blake2b<cardano_hash_32>(std::span<const uint8_t>(msg));
            expect(ed25519_verify(sig, vk, std::span(msg)) == false);
            expect(ed25519_verify(sig, vk, std::span(hash)) == true);
        };
    };
};
