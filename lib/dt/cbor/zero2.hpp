...
namespace daedalus_turbo::cbor::zero {
        ...
        size_t raw_size() const
        {
            if (!_size_final) {
                switch (const auto typ = type(); typ) {
                    ...
                    case major_type::tag: {
                        auto [id, v] = tag();
                        _set_size(_spec_bytes + v.raw_size());
                        break;
                    }
                }
                if (!_size_final) [[unlikely]]
                    throw error("cbor internal error: the size of an value has not been finalized!");
            }
            return _size;
        }
    ...
    protected:
        ...
        explicit value(const buffer data, decoder *parent=nullptr):
            _data { _check_data(data) }, _size { data.size() }, _parent { parent }
        {
            ...
            switch (const auto typ = type(); typ) {
                ...
                case major_type::simple:
                    _set_size(_spec_bytes);
                    break;
                case major_type::bytes:
                case major_type::text:
                    if (special() != special_val::s_break)
                        _set_size(_spec_uint + 1);
                    break;
                default:
                    break;
            }
        ...
        {
            return raw_span().subbuf(pos, sz);
        }

        buffer _subbuf(const size_t pos) const
        {
            return raw_span().subbuf(pos);
        }
    private:
        const uint8_t *_data;
        mutable uint64_t _size: 59 = 0;
        mutable uint64_t _size_final: 1 = 0;
        uint64_t _spec_bytes: 4 = 0;
        uint64_t _spec_uint;
        decoder *_parent = nullptr;
        ...
        void _set_size(const uint64_t sz) const
        {
            if (!_size_final) {
                if (sz > _size) [[unlikely]]
                    throw error("the final size must be greater than the buffer size: {}", sz, static_cast<uint64_t>(_size));
                _size = sz;
                _size_final = 1;
                _parent->_advance(_size);
            }
        }
    };
