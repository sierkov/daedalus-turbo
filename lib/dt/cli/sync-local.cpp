...
namespace daedalus_turbo::cli::sync_local {
    struct cmd: command {
        void configure(config &cmd) const override
        {
            ...
            cmd.opts.try_emplace("store", "do not validate or index data");
        ...
        void run(const arguments &args, const options &opts) const override
        {
            ...
            chunk_registry::mode mode = chunk_registry::mode::validate;
            if (const auto opt_it = opts.find("store"); opt_it != opts.end())
                mode = chunk_registry::mode::store;
