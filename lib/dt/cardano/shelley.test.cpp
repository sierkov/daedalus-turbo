    ...
    "cardano::shelley"_test = [] {
        ...
        "bootstrap hash"_test = [] {
...

            {
                ...
                enc.bytes(cc);
                enc.bytes(attrs);
                test_same(true, b_addr.bootstrap_ok(cbor::parse(enc.cbor())));
            }
        };
        "bootstrap witness"_test = [] {
            ...
            for (size_t i = 0; i < test_vecs.size(); ++i) {
                ...
                test_same(fmt::format("test vector #{}: {}", i, test.addr), true, addr.bootstrap_ok(wit));
            }
        };
    };
};