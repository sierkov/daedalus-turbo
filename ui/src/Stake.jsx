...
export default function Stake() {
    ...
    useEffect(() => {
        if (infoHash !== hash) {
            infoHash = hash;
            setInfo({});
            appAPI.stakeInfo(hash).then(newInfo => setInfo(newInfo));
        }
    }, [params]);
    ...
    useEffect(() => {
        if (cachedTxOffset != txOffset) {
            ...
            const oldInfo = { ... info };
            setInfo({});
            appAPI.stakeTxs(hash, txOffset, txLimit).then(newTxs => setInfo({ ...oldInfo, transactions: newTxs?.transactions }));
        }
    }, [txOffset]);
    ...
    useEffect(() => {
        if (cachedAssetOffset != assetOffset) {
            ...
            const oldInfo = { ... info };
            setInfo({});
            appAPI.stakeAssets(hash, assetOffset, assetLimit).then(newAssets => setInfo({ ...oldInfo, assets: newAssets?.assets }));
        }
    }, [assetOffset]);
        ...
        return <div className="content">
            ...
            <div className="stake-key tx">
                <div className="tx-header">
                    <h3>Error</h3>
                    <p>Couldn't retrieve data for stake key {hash}: {info.error}</p>
                </div>
            </div>
        </div>;
